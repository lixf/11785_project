...
def model(sess, hps, train_iterator, test_iterator, data_init):
    ...
    def sample(_y, _eps_std):
        x = m.sess.run(x_sampled, {Y: _y, m.eps_std: _eps_std})
        return x

    m.sample = sample

    #sample complete
    def f_sample_complete(y, eps_std):
        with tf.variable_scope('model', reuse=True):
            y_onehot = tf.cast(tf.one_hot(y, hps.n_y, 1, 0), 'float32')
    ...
    #m.eps_std = tf.placeholder(tf.float32, [None], name='eps_std')
    x_sampled_complete = f_sample_complete(Y, m.eps_std)

    def sample_complete(_y, _eps_std):
        x = m.sess.run(x_sampled_complete, {Y: _y, m.eps_std: _eps_std})
        return x

    m.sample_complete = sample_complete
