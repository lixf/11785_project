...
def init_visualizations(hps, model, logdir):
    ...
    def draw_samples(epoch):
        ...
        rows = 10 if image_size <= 64 else 4
        cols = rows
        n_batch = rows * cols
        ...
        for i in range(len(x_samples)):
            x_sample = np.reshape(
                x_samples[i], (n_batch, image_size, image_size, 3))
...
def infer(sess, model, hps, iterator):
    ...
    for it in range(hps.full_test_its):
        if hps.direct_iterator:
            # replace with x, y, attr if you're getting CelebA attributes, also
            # modify get_data
...
def train(sess, model, hps, logdir, visualise):
    ...
    for epoch in range(1, hps.epochs):
        ...
        for it in range(hps.train_its):
            ...
            if it % 500 == 0:
                print("Iter", it)
            # Set learning rate, linearly annealed from 0 in the first
            # hps.epochs_warmup epochs.
            lr = hps.lr * min(1., n_processed /
                              (hps.n_train * hps.epochs_warmup))

            # Run a training step synchronously.
            _t = time.time()
            result = model.train(lr)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs_full_valid", type=int,
                        default=10, help="Epochs between valid")
